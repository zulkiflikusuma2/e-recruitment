<div class="modal fade" id="modaledit{{$job->slug}}">
    <div class="modal-dialog modal-lg">
        <form action="/activevacancies/{{ $job->slug }}" method="post" class="mb-5" enctype="multipart/form-data">
            @method('put')
            @csrf
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="/activevacancies/">Lowongan Aktif</a></li>
    <li class="breadcrumb-item active"><a href="/activevacancies/{{ $job->slug }}/edit">Ubah Lowongan</a></li>
@endsection

@section('container')

    <!-- Tambah Pengajuan Lowongan -->
        <div class="card-header py-3">
            <h5 class="m-0 font-weight-bold text-primary" style="text-align: center">Ubah Lowongan</h5>
        </div>
        <div class="card-body" style="margin-left:15%;">
            <div class="col-lg-10">
                <form action="/activevacancies/{{ $job->slug }}" method="post" class="mb-5" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="slug" id="slug">
                    <div class="mb-4 mt-3">
                        <label for="requirement" class="form-label">Kualifikasi</label>
                        @error('requirement')
                            <p class="text-danger"> {{ $message }} </p>
                        @enderror
                        <input id="requirement" type="hidden" name="requirement" value="{{ old('requirement', $job->requirement) }}">
                    ...
                    <div style="margin-left: 38%">
                        <button type="submit" class="btn btn-primary">Ubah Lowongan</button>
                    </div>
                </form>
            </div>
        </div>
