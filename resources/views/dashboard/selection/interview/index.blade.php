...
@section('title', 'Interview | CBI')
@section('breadcrumb')
    <li class="breadcrumb-item">
        <a href="/dashboard">Dashboard</a>
    </li>
    <li class="breadcrumb-item"><a href="/interview">Tahap Seleksi Wawancara</a></li>
@endsection
...
@section('container')
    ...
    <ul class="nav nav-pills">
        ...
        <li><a href="/interview" class="nav-link active">Wawancara</a>
        </li>
    </ul>
    ...
    <form action="" method="post" class="d-inline">
        @method('patch')
        @csrf
        @include('dashboard.selection.interview.confirm')
        @include('dashboard.selection.interview.announ')
                    ...
                    @if ($applicants->count())
                        @foreach ($applicants as $applicant)
                            <tr align="center">
                                ...
                                {{-- <td>{{ $applicant->practice_test->written_test->score }}</td>
                      <td>{{ $applicant->practice_test->score }}</td> --}}
                                ...
                                <td>
                                    <a href="/interview/{{ $applicant->id }}" class="btn btn-info" data-toggle="modal"
                                        data-target="#modalshow{{ $applicant->id }}"><i class="fas fa-eye"></i></a>
                                    <a href="/interview/{{ $applicant->id }}/edit" class="btn btn-warning"
                                        data-toggle="modal" data-target="#modaledit{{ $applicant->id }}"><i
                                            class="fas fa-plus"></i></a>
                                    {{-- <a href="/interview/{{ $applicant->id }}" class="btn btn-danger"  data-toggle="modal" data-target="#modaldelete{{ $applicant->id }}"><i class="fas fa-trash"></i></a> --}}
                                </td>
                            </tr>
    </form>
    @include('dashboard.selection.interview.edit')
    @include('dashboard.selection.interview.show')
    @endforeach
    @endif
    </tbody>
    </table>
